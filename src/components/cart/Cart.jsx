...
const CartItem = ({ item, onRemove, onUpdateQuantity }) => (
  <li className="flex flex-col py-2 border-b border-gray-700">
    <div className="flex justify-between items-center">
      <span className="font-medium truncate max-w-[140px]">{item.name}</span>
      <div className="flex items-center space-x-2">
        <span className="text-gray-300">{item.price} DKK</span>
        <button
    ...
    <div className="flex justify-between items-center mt-1">
      ...
      <span className="font-semibold">{item.price * item.quantity} DKK</span>
    </div>
  </li>
);
...
const Cart = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      <div
        id="cart-panel"
        className={`absolute bottom-16 right-0 w-80 bg-black text-white rounded-lg shadow-xl transition-all duration-300 overflow-hidden ${
          isOpen
        ...
        {cartItems.length > 0 && (
          <div className="p-4 bg-gray-900">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-lg font-bold">{total} DKK</span>
            </div>

            {/* Checkout Button */}
            <Link href="/pages/checkout">
              <button className="w-full bg-white text-black py-2 px-4 rounded-md hover:bg-lime-100 transition-colors font-medium flex items-center justify-center">
                Proceed to Checkout
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
